...
public:
    WebcamPublisher() : Node("webcam_publisher") {
        ...
        timer_ = this->create_wall_timer(std::chrono::milliseconds(100), std::bind(&WebcamPublisher::publishFrame, this));
    }
...
private:
    ...
    void publishFrame() {
        ...
        RCLCPP_INFO(this->get_logger(), "Frame dimensions: x=%d, y=%d, type=%d", frame.cols, frame.rows, frame.type());
